 ...
 */
package ua.mobius.media.server.impl.dsp.audio.gsm;
...
public class Decoder {
    ...
    public ShortFrame process(ByteFrame frame)
    {
    	...
    	if(data.length==0)
    	{
    		...
    		ShortFrame res = ShortMemory.allocate(160);
    	...
    	ShortFrame res = ShortMemory.allocate(160*framesCount);
    	resdata=res.getData();
    	for(l=0;l<framesCount;l++)
    	{
    		k_temp=(short)(l*33);
    		...
    		k_temp=(short)(l*160);
    		System.arraycopy(result, 0, resdata, k_temp, 160);    		
    	}
    	
        res.setOffset(0);
        res.setLength(160*framesCount);
        res.setTimestamp(frame.getTimestamp());
        res.setDuration(frame.getDuration());
        res.setSequenceNumber(frame.getSequenceNumber());
        res.setEOM(frame.isEOM());
