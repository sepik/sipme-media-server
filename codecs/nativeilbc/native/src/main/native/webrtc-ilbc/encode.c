...
void WebRtcIlbcfix_EncodeImpl(
  ...
  if (iLBCenc_inst->section != 0){
    ...
    WebRtcIlbcfix_LpcEncode(syntdenum, weightdenum, iLBCbits_inst->lsf, data,
                            iLBCenc_inst->lpc_buffer,iLBCenc_inst->lsfold,iLBCenc_inst->lsfdeqold);
    ...
    iLBCbits_inst->startIdx = WebRtcIlbcfix_FrameClassify(residual);
    ...
    iLBCbits_inst->idxForMax=WebRtcIlbcfix_StateSearch(iLBCbits_inst->state_first, &residual[start_pos],
                              &syntdenum[(iLBCbits_inst->startIdx-1)*(LPC_FILTERORDER+1)],
                              &weightdenum[(iLBCbits_inst->startIdx-1)*(LPC_FILTERORDER+1)],iLBCbits_inst->idxVec);
    ...
    if (iLBCbits_inst->state_first) { /* put adaptive part in the end */
      ...
      WebRtcIlbcfix_CbSearch(iLBCbits_inst->cb_index, iLBCbits_inst->gain_index,
  ...
  if( Nfor > 0 ){
    ...
    for (subframe = start_count; subframe < end_count; subframe++){
      ...
      WebRtcIlbcfix_CbSearch(iLBCbits_inst->cb_index+subcount*CB_NSTAGES,
