  ...
  if (mode) { /* the data are good */
    ...
    if (mode) { /* No bit errors was detected, continue decoding */
      ...
      WebRtcIlbcfix_DecoderInterpolateLsp(syntdenum, weightdenum,
                                          lsfdeq, LPC_FILTERORDER, iLBCdec_inst->mode,iLBCdec_inst->lsfdeqold);

      /* Decode the residual using the cb and gain indexes */
      WebRtcIlbcfix_DecodeResidual(iLBCdec_inst->prevResidual,iLBCdec_inst->enh_buf, iLBCbits_inst->idxVec,iLBCbits_inst->cb_index,iLBCbits_inst->gain_index, decresidual, syntdenum,iLBCdec_inst->state_short_len,iLBCdec_inst->nsub,iLBCbits_inst->state_first,iLBCbits_inst->startIdx,iLBCbits_inst->idxForMax);
