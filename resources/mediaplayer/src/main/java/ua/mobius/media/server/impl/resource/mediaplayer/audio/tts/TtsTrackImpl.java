 ...
 */
package ua.mobius.media.server.impl.resource.mediaplayer.audio.tts;
...
public class TtsTrackImpl implements Track {
    ...
    private AudioFormat format = FormatFactory.createAudioFormat("l16", 8000, 16,1);
...
//        frameSize = (int) (period * format.getChannels() * format.getSampleSize() * format.getSampleRate() / 8000);
    ...
    private class TTSAudioBuffer implements
        ...
        public boolean end() {
            ...
            logger.debug("end() called totalBytes = " + totalBytes);
        ...
        public void flip() {
           ...
           //duration = (long) (totalBytes * 1000 / (fmt.getSampleSizeInBits() / 8 * fmt.getSampleRate()));
            duration=totalBytes*period*1000000L/frameSize;
